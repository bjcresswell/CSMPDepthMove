...
```{r}
# Load data
load(file = "data/Rdata/shark_individual_daily_tds.Rdata")
```
...
### Individual shark (transmitter_id) included as a random effect
...
shark_individual_daily_td %>% 
  ...
  facet_wrap( ~ transmitter_id, ncol = 5)
```

Black line = daily maximum depth, Red line = daily depth SD
...
```{r echo = FALSE, include = TRUE, message = FALSE, warning = FALSE}
sst_maxd_final %>% emmeans(~ SST)
sst_maxd_final %>% emtrends("Sex", var = "SST")# %>% contrast()
```
