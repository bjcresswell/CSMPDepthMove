...
# PUT THE DEPTH OVERVIEW PLOT IN HERE
...
# Fit model
```{r}
maxdepth_timeGAM <- gam(daily_max_depth ~ s(study_day, bs = 'cc', k = 15), data = shark_daily_td, knots = list(doy = c(0.5, 365.5)))
```
...
max_depth_gam_plot <- 
    ggplot() +
    geom_point(data = shark_daily_td, aes(x = study_day, y = daily_max_depth), pch = 21, alpha = 0.5) +
    geom_ribbon(data = maxdepthgam_newdata, aes(y = emmean, x = study_day, ymin=lower.CL, ymax=upper.CL), color=NA, alpha=0.8) +
    geom_line(data = maxdepthgam_newdata, aes(y = emmean, x = study_day)) +
    ylab("Depth (m)") + 
    xlab("Day of year") +
    scale_y_reverse() +
    theme_minimal() +
...
## NEED TO CHANGE FROM HERE DOWN TO DEPTH ##
