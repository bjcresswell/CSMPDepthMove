...
#shark_daily_temps_plot <- 
  ...
  geom_smooth(aes(x = date, y = daily_max_temp), colour = "red", method = "gam",  formula = y ~ s(x, bs = 'cs'), level = 0.95) +
  geom_line(aes(x = date, y = daily_min_temp), colour = "midnightblue") +
  geom_smooth(aes(x = date, y = daily_min_temp), colour = "midnightblue", method = "gam",  formula = y ~ s(x, bs = 'cs'), level = 0.95) +
  geom_line(aes(x = date, y = daily_mean_temp), colour = "grey40") +
  geom_smooth(aes(x = date, y = daily_mean_temp), colour = "grey40", method = "gam",  formula = y ~ s(x, bs = 'cs'), level = 0.95) +
...
We need to formally model these trends with purpose-build GAMs and identify peaks/troughs 


# The date variable won't play nicely with GAM (or other way around) so need to make a new variable called study_day and use cubic spline
```{r}
shark_daily_td <- 
  shark_daily_td %>% 
...
# Fit model
```{r}
mintemp_timeGAM <- gam(daily_min_temp ~ s(study_day, bs = 'cc'), data = shark_daily_td, knots = list(doy = c(0.5, 365.5)))
```
...
## Results
```{r}
#(gamresults <- mintemp_timeGAM %>% tidy(conf.int=TRUE))
...
time_temp_gam_plot <- 
    ggplot() +
    geom_point(data = shark_daily_td, aes(x = study_day, y = daily_min_temp), pch = 21, alpha = 0.5) +
    geom_ribbon(data = mintempgam_newdata, aes(y = emmean, x = study_day, ymin=lower.CL, ymax=upper.CL), color=NA, alpha=0.8) +
    geom_line(data = mintempgam_newdata, aes(y = emmean, x = study_day)) +
    ylab("Temp (°C)") + 
    xlab("Day of year") +
