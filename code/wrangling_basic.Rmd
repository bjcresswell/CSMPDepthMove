...
```{r load-dep2}
data_dep2 <- read_excel('../data/CSMPdb_uncal_202203.xlsx') %>% 
...
head(data_dep2)
data_dep2 %$% 
  tibble(unique(receiver_name))
```

# Do max and min dates make sense
```{r}
data_dep2 %$% 
  summary(as_datetime(DTG))
```
...
osprey_dep2 <- data_dep2 %>%  
  left_join(vr2ws_dep2, by='receiver_name') #%>% 
...
## Deployment 3 (Oct 2021 - Oct 2022 -> BJC units)

## Load detection database 3
```{r load-dep1}
data_dep3 <- read.csv('../data/CSMPdb_uncal_202210.csv') %>% 
  tibble() %>% # Make into tibble format
...
```{r}
osprey_dep3 %>% 
  filter(detection_timestamp < '2020-01-01 22:07:31.0000') %$% 
